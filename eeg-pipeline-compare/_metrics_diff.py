...
def _compute_CR_diff(m1: dict, m2: dict) -> dict:
    ...
    return {
        "n_bad": _scalar_diff(cr1.get("n_bad"), cr2.get("n_bad")),
        "rate": _scalar_diff(cr1.get("rate"), cr2.get("rate")),
...
def _compute_WR_diff(m1: dict, m2: dict) -> dict:
    ...
    return {
        "rejection_rate": _scalar_diff(wr1.get("rejection_rate"), wr2.get("rejection_rate")),
        "n_rejected": _scalar_diff(wr1.get("n_rejected"), wr2.get("n_rejected")),
    }
...
def _compute_ICA_diff(m1: dict, m2: dict) -> dict:
    ...
    return {
        "n_components": _scalar_diff(i1.get("n_components"), i2.get("n_components")),
        "n_excluded": _scalar_diff(i1.get("n_excluded"), i2.get("n_excluded")),
        "exclusion_rate": _scalar_diff(i1.get("exclusion_rate"), i2.get("exclusion_rate")),
    }
...
def _compute_PSD_diff(m1: dict, m2: dict) -> dict:
    ...
    result = {
        band: _scalar_diff(p1.get(band), p2.get(band))
        for band in ("delta", "theta", "alpha", "beta", "gamma")
    }
    result["line_noise_50hz"] = _scalar_diff(
        m1.get("line_noise_50hz"), m2.get("line_noise_50hz")
    )
    return result
